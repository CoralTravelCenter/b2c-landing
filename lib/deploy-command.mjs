...
export const handler = (async argv => {
    ...
    if (argv.assets) {
        ...
        const isRepo = await asyncTask('Git check', new Promise(async (resolve, reject) => {
            const is_repo = await git.checkIsRepo('root');
            is_repo ? resolve(true) : reject('No git repo found');
        }));

        console.log(pageDescriptor);
