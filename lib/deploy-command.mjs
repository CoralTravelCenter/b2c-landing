...
export const handler = (async argv => {
    ...
    if (!deploymentDescriptor || argv.newPage) {
        ...
        if (isLocalContentVersionLatest) {
            if (argv.newVersion || [statusPublished, statusUnpublished].includes(latest.statusId)) {
...
async function obtainContentToUpdate(backoffice, localPageContentId) {
    ...
    return await asyncTask('Fetching page content', backoffice.api.getContent(chosenPageContentId));
}
