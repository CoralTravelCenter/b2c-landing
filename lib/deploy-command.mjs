...
export const handler = (async argv => {
    ...
    if (!deploymentDescriptor || argv.N) {
        ...
        const widget_queries = addWidgetsQueries(landingName, pageDescriptor, pageContentId, newPageParams.layoutAreaId);
        for (const widgetQuery of widget_queries) {
            const result = await backoffice.api.addWidget(widgetQuery);
            console.log(`added ${ widgetQuery.cmsTitle }`);
            console.log(result);
        }
...
function addWidgetsQueries(landingName, pageDescriptor, pageContentId, layoutAreaId) {
    const buildDir = path.join(process.cwd(), '@CMS');
