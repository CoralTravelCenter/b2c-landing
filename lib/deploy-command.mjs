...
import {
    ...
    hasBeenBuilt, idsFromPageContentId, storeAssetsDescriptor,
...
export const handler = (async argv => {
    ...
    if (argv.assets || argv.assetsOnly) {
        ...
        if (assets2deploy.length) {
            const uploaded_assets = await deployAssets(backoffice.api, cdn_project_pathname, assets2deploy, 1);
...
function enqueueAssetUpload(api, cdn_project_pathname, upload_queue, assets_list, results) {
    ...
    if (asset) {
        const spinner = ora();
        const mime_type = mime.getType(asset.absolutePath);
        ...
        const promise = api.uploadFile(
            ...
            new Blob([fs.readFileSync(asset.absolutePath)], { type: mime_type })
        ).then(upload_result => {
