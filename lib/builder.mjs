...
export async function parcelAndWatchSource(source_abs_path, env, watcher) {
    ...
    env = Object.assign({}, page_descriptor.env, env);
...
function randomParcelRequireGlobal(markup) {
    return markup.replace(/"parcelRequire([0-9a-f]+)"/, "$& + '_' + Math.floor(Math.random()*10000)");
}
...
function widgetsFromBuildEvent(buildEvent, env) {
    ...
    const widgets = html_bundles.map(hb => {
        ...
        if (env.mode === 'production') {
            ...
            markup = randomParcelRequireGlobal(markup);
        }
        return {
            naming: {
                brand:  env.brand,
