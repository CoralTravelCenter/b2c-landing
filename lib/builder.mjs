...
import { pageDescriptorWithFile } from "./utils.mjs";
...
function replaceAssetsRefs(markup, env) {
    return markup.replaceAll(env.localAssetsPath, env.cdnAssetsURL);
}
...
function widgetsFromBuildEvent(buildEvent, env) {
    ...
    const widgets = html_bundles.map(hb => {
        ...
        const css_tags = Array.from(css_nodes, css_node => {
            ...
            const ref_bundle = bundles.find(bundle => bundle.filePath.includes(href));
        ...
        const script_tags = Array.from(script_nodes, script_node => {
            ...
            const ref_bundle = bundles.find(bundle => bundle.filePath.includes(src));
            if (!ref_bundle) debugger;
            script_node.remove();
